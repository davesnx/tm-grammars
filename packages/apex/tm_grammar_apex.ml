...
let json =
  ...
  "repository": {
    ...
    "date-literal-with-params": {
      ...
      "match": "\\b(((?:LAST_N_DAY|NEXT_N_DAY|NEXT_N_WEEK|LAST_N_WEEK|NEXT_N_MONTH|LAST_N_MONTH|NEXT_N_QUARTER|LAST_N_QUARTER|NEXT_N_YEAR|LAST_N_YEAR|NEXT_N_FISCAL_QUARTER|LAST_N_FISCAL_QUARTER|NEXT_N_FISCAL_YEAR|LAST_N_FISCAL_YEAR)S)\\s*:\\d+)\\b"
    },
    "date-literals": {
      "captures": {
        "1": {
    ...
    "local-constant-declaration": {
      "begin": "\\b(?<const_keyword>const)\\b\\s*(?<type_name>(?:(?<identifier>@?[_[:alpha:]][_[:alnum:]]*)\\s*::\\s*)?(?<name_and_type_args>\\g<identifier>\\s*(?<type_args>\\s*<(?:[^<>]|\\g<type_args>)+>\\s*)?)(?:\\s*\\.\\s*\\g<name_and_type_args>)*(?:\\s*\\?\\s*)?(?:\\s*\\[(?:\\s*,\\s*)*]\\s*)*)\\s+(\\g<identifier>)\\s*(?=[,;=])",
      "beginCaptures": {
        "1": {
    ...
    "local-variable-declaration": {
      "begin": "(?:(?:\\b(ref)\\s+)?\\b(var)\\b|(?<type_name>(?:ref\\s+)?(?:(?<identifier>@?[_[:alpha:]][_[:alnum:]]*)\\s*::\\s*)?(?<name_and_type_args>\\g<identifier>\\s*(?<type_args>\\s*<(?:[^<>]|\\g<type_args>)+>\\s*)?)(?:\\s*\\.\\s*\\g<name_and_type_args>)*(?:\\s*\\?\\s*)?(?:\\s*\\[(?:\\s*,\\s*)*]\\s*)*))\\s+(\\g<identifier>)\\s*(?=[),;=])",
      "beginCaptures": {
        "1": {
          "name": "storage.modifier.apex"
        },
    ...
    "numeric-literal": {
      "patterns": [
        ...
        {
          "match": "\\b0([Xx])[_\\h]+([LUlu]|UL|Ul|uL|ul|LU|Lu|lU|lu)?\\b",
          "name": "constant.numeric.hex.apex"
        },
        {
          "match": "\\b0([Bb])[01_]+([LUlu]|UL|Ul|uL|ul|LU|Lu|lU|lu)?\\b",
          "name": "constant.numeric.binary.apex"
        },
        {
          "match": "\\b([0-9_]+)?\\.[0-9_]+(([Ee])[0-9]+)?([DFMdfm])?\\b",
          "name": "constant.numeric.decimal.apex"
        },
        {
          "match": "\\b[0-9_]+([Ee])[0-9_]+([DFMdfm])?\\b",
          "name": "constant.numeric.decimal.apex"
        },
        {
          "match": "\\b[0-9_]+([DFMdfm])\\b",
          "name": "constant.numeric.decimal.apex"
        },
        {
          "match": "\\b[0-9_]+([LUlu]|UL|Ul|uL|ul|LU|Lu|lU|lu)?\\b",
          "name": "constant.numeric.decimal.apex"
        }
      ]
    },
    ...
    "sharing-modifier": {
      "match": "(?<!\\.)\\b((?:with|without|inherited) sharing)\\b",

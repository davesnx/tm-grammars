...
let json =
  ...
  "patterns": [
    ...
    {
      "match": "(?i)(^|\\s+)((P(?:UBLIC|RIVATE|ROTECTED))\\sSECTION)(?=\\s+|[.:])",
    ...
    },
    {
      ...
      "patterns": [
        {
          "match": "(?i)(^|\\s+)(definition|implementation|public|inheriting\\s+from|final|deferred|abstract|shared\\s+memory\\s+enabled|(global|local)*\\s*friends|(create\\s+(p(?:ublic|rotected|rivate)))|for\\s+behavior\\s+of|for\\s+testing|risk\\s+level\\s+(critical|dangerous|harmless))|duration\\s(short|medium|long)(?=\\s+|\\.)",
          "name": "storage.modifier.class.abap"
        },
        {
    ...
    {
      "begin": "(?i)^\\s*(method)\\s(?:([/_a-z][/-9_a-z]*)~)?([/_a-z][/-9_a-z]*)",
      "beginCaptures": {
        "1": {
          "name": "storage.type.block.abap"
        },
      ...
      "patterns": [
        {
          "match": "(?i)(?<=^|\\s)(BY(?:\\s+DATABASE(\\s+PROCEDURE|\\s+FUNCTION|\\s+GRAPH\\s+WORKSPACE)|\\s+KERNEL\\s+MODULE))(?=\\s+|\\.)",
        ...
        },
        {
          ...
          "match": "(?i)(?<=\\s)(USING)\\s+([/_a-z][/-9=>_a-z]*)+(?=\\s+|\\.)"
        },
        {
          "begin": "(?=[A-Z_a-z][0-9A-Z_a-z]*)",
          "end": "(?![0-9A-Z_a-z])",
    ...
    {
      "begin": "(?i)^\\s*(INTERFACE)\\s([/_a-z][/-9_a-z]*)",
      "beginCaptures": {
        "1": {
          "name": "storage.type.block.abap"
        },
    ...
    {
      "match": "(?i)(end(?:class|method|form|interface))",
      "name": "storage.type.block.end.abap"
    },
    {
      "match": "(?i)(<[A-Z_a-z][0-9A-Z_a-z]*>)",
  ...
  "repository": {
    ...
    "abaptypes": {
      "patterns": [
        {
          "match": "(?i)\\s(abap_bool|string|xstring|any|clike|csequence|numeric|xsequence|decfloat|decfloat16|decfloat34|utclong|simple|int8|[cdfinptx])(?=[,.\\s])",
    ...
    "comparison_operator": {
      "match": "(?i)(?<=\\s)([<>]|<=|>=|=|<>|eq|ne|lt|le|gt|ge|cs|cp|co|cn|ca|na|ns|np|byte-co|byte-cn|byte-ca|byte-na|byte-cs|byte-ns|[moz])(?=\\s)",
      "name": "keyword.control.simple.abap"
    },
    "control_keywords": {
    ...
    "keywords_followed_by_braces": {
      ...
      "match": "(?i)\\b(data|value|field-symbol|final|reference|resumable)\\((<?[/_a-z][/-9_a-z]*>?)\\)"
    },
    "logical_operator": {

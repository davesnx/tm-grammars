...
let json = {json|{
  ...
  "repository": {
    ...
    "constants": {
      "patterns": [
        {
          "begin": "(?i)(?=((\\\\[_a-z][0-9_a-z]*\\\\[_a-z][0-9\\\\_a-z]*)|([_a-z][0-9_a-z]*\\\\[_a-z][0-9\\\\_a-z]*))[^0-9\\\\_a-z])",
          "end": "(?i)([_a-z][0-9_a-z]*)?(?=[^0-9\\\\_a-z])",
          "endCaptures": {
            "1": {
        ...
        },
        {
          ...
          "patterns": [
            ...
            {
              "match": "[A-Z_a-z\\x7F-ÿ][0-9A-Z_a-z\\x7F-ÿ]*",
              "name": "constant.other.php"
            }
          ]
        }
    ...
    "function-arguments": {
      "patterns": [
        ...
        {
          "begin": "(?i)((\\$+)[_a-z\\x7F-ÿ][0-9_a-z\\x7F-ÿ]*)",
          "beginCaptures": {
            "1": {
      ...
      "patterns": [
        {
          ...
          "patterns": [
            ...
            {
              "match": "([-+])?([A-Z_a-z][0-9A-Z_a-z]*)(?:\\s+(as|super)\\s+([A-Z_a-z][0-9A-Z_a-z]*))?",
    ...
    "heredoc": {
      "patterns": [
        {
          "begin": "<<<\\s*(\"?)([A-Z_a-z]+[0-9A-Z_a-z]*)(\\1)\\s*$",
          "beginCaptures": {
            "2": {
    ...
    "instantiation": {
      ...
      "patterns": [
        {
          "match": "(parent|static|self)(?=[^0-9_a-z])",
          "name": "support.type.php"
        },
        {
          "include": "#class-name"
    ...
    "interpolation": {
      "patterns": [
        ...
        {
          "match": "(\\$[A-Z_a-z][0-9A-Z_a-z]*((->[A-Z_a-z][0-9A-Z_a-z]*)|(\\[[0-9A-Z_a-z]+]))?)",
          "name": "variable.other.php"
        }
      ]
    },
    ...
    "language": {
      "patterns": [
        ...
        {
          "begin": "(?i)^\\s*(?:(module)\\s*)?((?:|new)type)\\s+([0-9_a-z]+)",
          "beginCaptures": {
            "1": {
              "name": "storage.modifier.php"
            },
        ...
        {
          "begin": "(?i)^\\s*(?:(public|internal)\\s+)?(enum)\\s+(class)\\s+([0-9_a-z]+)\\s*:?",
          "beginCaptures": {
            "1": {
              "name": "storage.modifier.php"
            },
        ...
        {
          "begin": "(?i)^\\s*(?:(public|internal)\\s+)?(enum)\\s+([0-9_a-z]+)\\s*:?",
          "beginCaptures": {
            "1": {
              "name": "storage.modifier.php"
            },
        ...
        {
          "begin": "(?i)^\\s*(?:(public|internal)\\s+)?(trait)\\s+([0-9_a-z]+)\\s*",
          "beginCaptures": {
            "1": {
              "name": "storage.modifier.php"
            },
        ...
        {
          "begin": "^\\s*(new)\\s+(module)\\s+([.0-9A-Z_a-z]+)\\b",
          "beginCaptures": {
            "1": {
        ...
        {
          "begin": "(?i)(?:^\\s*|\\s*)(namespace)\\b\\s+(?=([0-9\\\\_a-z]*\\s*($|[;{]|(/[*/])))|$)",
          "beginCaptures": {
            "1": {
        ...
        {
          "begin": "(?i)^\\s*((?:(?:final|abstract|public|internal)\\s+)*)(class)\\s+([0-9_a-z]+)\\s*",
          "beginCaptures": {
            "1": {
              "patterns": [
                {
        ...
        },
        {
          ...
          "patterns": [
            ...
            },
            {
              ...
              "match": "(?i)([_a-z\\x7F-\\x{10FFFF}][0-9_a-z\\x7F-\\x{10FFFF}]*)((?:\\s*\\|\\s*[_a-z\\x7F-\\x{10FFFF}][0-9_a-z\\x7F-\\x{10FFFF}]*)*)\\s*((\\$+)[_a-z\\x7F-\\x{10FFFF}][0-9_a-z\\x7F-\\x{10FFFF}]*)"
            }
          ]
        },
        {
              ...
              "patterns": [
                {
                  ...
                  "match": "(?:\\s*(&))?\\s*((\\$+)[A-Z_a-z\\x7F-ÿ][0-9A-Z_a-z\\x7F-ÿ]*)\\s*(?=[),])",
        ...
        {
          "begin": "\\s*((?:(?:final|abstract|public|private|protected|internal|static|async)\\s+)*)(function)\\s+(?:(__(?:call|construct|destruct|get|set|isset|unset|tostring|clone|set_state|sleep|wakeup|autoload|invoke|callStatic|dispose|disposeAsync)(?=[^0-9A-Z_a-z\\x7F-ÿ]))|([0-9A-Z_a-z]+))",
          "beginCaptures": {
            "1": {
              "patterns": [
                {
        ...
        {
          "begin": "(?i)\\s*(?=[$0-9\\\\_a-z]+(::)(?:([_a-z][0-9_a-z]*)\\s*\\(|((\\$+)[_a-z\\x7F-ÿ][0-9_a-z\\x7F-ÿ]*)|([_a-z\\x7F-ÿ][0-9_a-z\\x7F-ÿ]*))?)",
          "end": "(::)(?:([A-Z_a-z][0-9A-Z_a-z]*)\\s*\\(|((\\$+)[A-Z_a-z\\x7F-ÿ][0-9A-Z_a-z\\x7F-ÿ]*)|([A-Z_a-z\\x7F-ÿ][0-9A-Z_a-z\\x7F-ÿ]*))?",
          "endCaptures": {
            "1": {
    ...
    "object": {
      "patterns": [
        ...
        },
        {
          ...
          "match": "(->)(?:([A-Z_a-z][0-9A-Z_a-z]*)\\s*\\(|((\\$+)?[A-Z_a-z\\x7F-ÿ][0-9A-Z_a-z\\x7F-ÿ]*))?"
        }
      ]
    },
    "parameter-default-types": {
      "patterns": [
        ...
        {
          "begin": "(?i)\\s*(?=[0-9\\\\_a-z]+(::)([_a-z\\x7F-ÿ][0-9_a-z\\x7F-ÿ]*)?)",
          "end": "(?i)(::)([_a-z\\x7F-ÿ][0-9_a-z\\x7F-ÿ]*)?",
          "endCaptures": {
            "1": {
              "name": "keyword.operator.class.php"
            },
    ...
    "support": {
      "patterns": [
        ...
        {
          "match": "(?i)\\b(com_(set|create_guid|i(senum|nvoke)|pr(int_typeinfo|op(set|put|get))|event_sink|load(_typelib)?|addref|release|get(_active_object)?|message_pump)|variant_(s(ub|et(_type)?)|n(ot|eg)|c(a(s?t)|mp)|i(nt|div|mp)|or|d(iv|ate_((?:to|from)_timestamp))|pow|eqv|fix|a(nd|dd|bs)|round|get_type|xor|m(od|ul)))\\b",
        ...
        {
          "match": "(?i)\\b(scandir|c(h(dir|root)|losedir)|opendir|dir|re((?:win|a)ddir)|getcwd)\\b",
        ...
        {
          "match": "(?i)\\benchant_(dict_(s(tore_replacement|uggest)|check|is_in_session|describe|quick_check|add_to_(session|personal)|get_error)|broker_(set_ordering|init|d(ict_exists|escribe)|free(_dict)?|list_dicts|request_((?:|pwl_)dict)|get_error))\\b",
        ...
        {
          "match": "(?i)\\b(set_e((?:rror|xception)_handler)|trigger_error|debug_((?:print_|)backtrace)|user_error|error_(log|reporting|get_last)|restore_e((?:rror|xception)_handler))\\b",
        ...
        {
          "match": "(?i)\\b(c(all_user_func(_array)?|reate_function)|unregister_tick_function|f(orward_static_call(_array)?|unc(tion_exists|_(num_args|get_arg(s)?)))|register_((?:shutdown|tick)_function)|get_defined_functions)\\b",
          "name": "support.function.funchand.php"
        },
        {
          "match": "(?i)\\b(ngettext|textdomain|d(ngettext|c(n?gettext)|gettext)|gettext|bind(textdomain|_textdomain_codeset))\\b",
          "name": "support.function.gettext.php"
        },
        {
          "match": "(?i)\\bgmp_(s(can([01])|trval|ign|ub|etbit|qrt(rem)?)|hamdist|ne(g|xtprime)|c(om|lrbit|mp)|testbit|in(tval|it|vert)|or|div(_(q(r)?|r)|exact)?|jacobi|p(o(pcount|w(m)?)|erfect_square|rob_prime)|fact|legendre|a(nd|dd|bs)|random|gcd(ext)?|xor|m(od|ul))\\b",
        ...
        {
          "match": "(?i)\\b(http_(s(upport|end_(st(atus|ream)|content_(type|disposition)|data|file|last_modified))|head|negotiate_(c(harset|ontent_type)|language)|c(hunked_decode|ache_(etag|last_modified))|throttle|inflate|d((?:efl|)ate)|p(ost_(data|fields)|ut_(stream|data|file)|ersistent_handles_(c(ount|lean)|ident)|arse_(headers|cookie|params|message))|re(direct|quest(_(method_(name|unregister|exists|register)|body_encode))?)|get(_request_(headers|body(_stream)?))?|match_(etag|request_header|modified)|build_(str|cookie|url))|ob_((?:inflate|deflate|etag)handler))\\b",
        ...
        {
          "match": "(?i)\\b(s(ys_get_temp_dir|et_(time_limit|include_path|magic_quotes_runtime))|ini_(set|alter|restore|get(_all)?)|zend_(thread_id|version|logo_guid)|dl|p(hp(credits|info|_(sapi_name|ini_(scanned_files|loaded_file)|uname|logo_guid)|version)|utenv)|extension_loaded|version_compare|assert(_options)?|restore_include_path|g(c_(collect_cycles|disable|enable(d)?)|et(opt|_(c(urrent_user|fg_var)|include(d_files|_path)|defined_constants|extension_funcs|loaded_extensions|required_files|magic_quotes_(runtime|gpc))|env|lastmod|rusage|my(inode|uid|pid|gid)))|m(emory_get_((?:|peak_)usage)|a(in|gic_quotes_runtime)))\\b",
          "name": "support.function.info.php"
        },
        {
          "match": "(?i)\\bibase_(se(t_event_handler|rv(ice_((?:de|at)tach)|er_info))|n(um_(params|fields)|ame_result)|c(o(nnect|mmit(_ret)?)|lose)|trans|d(elete_user|rop_db|b_info)|p(connect|aram_info|repare)|e(rr(code|msg)|xecute)|query|f(ield_info|etch_(object|assoc|row)|ree_(event_handler|query|result))|wait_event|a(dd_user|ffected_rows)|r(ollback(_ret)?|estore)|gen_id|m(odify_user|aintain_db)|b(lob_(c(lose|ancel|reate)|i(nfo|mport)|open|echo|add|get)|ackup))\\b",
          "name": "support.function.interbase.php"
        },
        {
          "match": "(?i)\\b(n(ormalizer_(normalize|is_normalized)|umfmt_(set_(symbol|text_attribute|pattern|attribute)|create|parse(_currency)?|format(_currency)?|get_(symbol|text_attribute|pattern|error_(code|message)|locale|attribute)))|collator_(s(ort(_with_sort_keys)?|et_(strength|attribute))|c(ompare|reate)|asort|get_(s(trength|ort_key)|error_(code|message)|locale|attribute))|transliterator_(create(_(inverse|from_rules))?|transliterate|list_ids|get_error_(code|message))|i(ntl_(is_failure|error_name|get_error_(code|message))|dn_to_(u(nicode|tf8)|ascii))|datefmt_(set_(calendar|timezone(_id)?|pattern|lenient)|create|is_lenient|parse|format(_object)?|localtime|get_(calendar(_object)?|time(type|zone(_id)?)|datetype|pattern|error_(code|message)|locale))|locale_(set_default|compose|parse|filter_matches|lookup|accept_from_http|get_(script|d(isplay_(script|name|variant|language|region)|efault)|primary_language|keywords|all_variants|region))|resourcebundle_(c(ount|reate)|locales|get(_error_(code|message))?)|grapheme_(s(tr(str|i(str|pos)|pos|len|r(i?pos))|ubstr)|extract)|msgfmt_(set_pattern|create|parse(_message)?|format(_message)?|get_(pattern|error_(code|message)|locale)))\\b",
        ...
        {
          "match": "(?i)\\b(s(in(h)?|qrt|rand)|h(ypot|exdec)|c(os(h)?|eil)|tan(h)?|is_(nan|infinite|finite)|octdec|de(c(hex|oct|bin)|g2rad)|p(i|ow)|exp(m1)?|f(loor|mod)|l(cg_value|og(1([0p]))?)|a(sin(h)?|cos(h)?|tan([2h])?|bs)|r(ound|a(nd|d2deg))|getrandmax|m(t_(srand|rand|getrandmax)|in|ax)|b(indec|ase_convert))\\b",
        ...
        {
          "match": "(?i)\\bm(crypt_(c(fb|reate_iv|bc)|ofb|decrypt|e(nc(_(self_test|is_block_(algorithm(_mode)?|mode)|get_(supported_key_sizes|iv_size|key_size|algorithms_name|modes_name|block_size))|rypt)|cb)|list_(algorithms|modes)|ge(neric(_(init|deinit|end))?|t_(cipher_name|iv_size|key_size|block_size))|module_(self_test|close|is_block_(algorithm(_mode)?|mode)|open|get_(supported_key_sizes|algo_((?:key|block)_size))))|decrypt_generic)\\b",
        ...
        {
          "match": "(?i)\\b(s(yslog|ocket_(set_(timeout|blocking)|get_status)|et((?:|raw)cookie))|h(ttp_response_code|eader(s_(sent|list)|_re(gister_callback|move))?)|c(heckdnsrr|loselog)|i(net_(ntop|pton)|p2long)|openlog|d(ns_(check_record|get_(record|mx))|efine_syslog_variables)|pfsockopen|fsockopen|long2ip|get(servby(name|port)|host(name|by(name(l)?|addr))|protobyn(umber|ame)|mxrr))\\b",
          "name": "support.function.network.php"
        },
        {
          "match": "(?i)\\bnsapi_(virtual|re((?:sponse|quest)_headers))\\b",
        ...
        {
          "match": "(?i)\\bftp_(s(sl_connect|ystype|i([tz]e)|et_option)|n(list|b_(continue|put|f(put|get)|get))|c(h(dir|mod)|onnect|dup|lose)|delete|p(ut|wd|asv)|exec|quit|f(put|get)|login|alloc|r(ename|aw(list)?|mdir)|get(_option)?|m(dtm|kdir))\\b",
        ...
        {
          "match": "(?i)\\bmssql_(select_db|n(um_(fields|rows)|ext_result)|c(onnect|lose)|init|data_seek|pconnect|execute|query|f(ield_(seek|name|type|length)|etch_(object|field|a(ssoc|rray)|row|batch)|ree_(statement|result))|r(ows_affected|esult)|g(uid_string|et_last_message)|min_((?:error|message)_severity)|bind)\\b",
        ...
        {
          "match": "(?i)\\bposix_(s(trerror|et(sid|uid|pgid|e([gu]id)|gid))|ctermid|t(tyname|imes)|i(satty|nitgroups)|uname|errno|kill|access|get(sid|cwd|uid|_last_error|p(id|pid|w(nam|uid)|g(id|rp))|e([gu]id)|login|rlimit|g(id|r(nam|oups|gid)))|mk(nod|fifo))\\b",
        ...
        {
          "match": "(?i)\\bpspell_(s(tore_replacement|uggest|ave_wordlist)|new(_(config|personal))?|c(heck|onfig_(save_repl|create|ignore|d((?:ict|ata)_dir)|personal|r(untogether|epl)|mode)|lear_session)|add_to_(session|personal))\\b",
        ...
        {
          "match": "(?i)\\bsqlite_(s(ingle_query|eek)|has_(prev|more)|n(um_(fields|rows)|ext)|c(hanges|olumn|urrent|lose|reate_(function|aggregate))|open|u(nbuffered_query|df_((?:de|en)code_binary))|p(open|rev)|e(scape_string|rror_string|xec)|valid|key|query|f(ield_name|etch_(s(tring|ingle)|column_types|object|a(ll|rray))|actory)|l(ib(encoding|version)|ast_(insert_rowid|error))|array_query|rewind|busy_timeout)\\b",
      ...
      "patterns": [
        {
          ...
          "match": "(\\$+)[A-Z_a-z\\x7F-ÿ][0-9A-Z_a-z\\x7F-ÿ]*?\\b",
          "name": "variable.other.php"
        }
      ]
    },
    ...
    "variable-name": {
      "patterns": [
        ...
        },
        {
          ...
          "match": "((\\$)(?<name>[A-Z_a-z\\x7F-ÿ][0-9A-Z_a-z\\x7F-ÿ]*))(?:(->)(\\g<name>)|(\\[)(?:(\\d+)|((\\$)\\g<name>)|(\\w+))(]))?"
        },
        {
          "captures": {
            "1": {
          ...
          "match": "((\\$\\{)(?<name>[A-Z_a-z\\x7F-ÿ][0-9A-Z_a-z\\x7F-ÿ]*)(}))"
        }
      ]
    },
    "variables": {
    ...
    "xhp-tag-element-name": {
      "patterns": [
        {
          "begin": "\\s*(<)([_\\p{L}][-:\\p{L}\\p{Mn}\\p{Mc}\\d\\p{Nl}\\p{Pc}]*+)(?=[/>\\s])(?<!:)",
          "beginCaptures": {
            "1": {

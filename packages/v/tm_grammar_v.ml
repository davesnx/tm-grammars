...
let json =
  ...
  "repository": {
        ...
        },
        {
          "patterns": [
            ...
            },
            {
              ...
              "match": "(bool|byte|byteptr|charptr|voidptr|string|rune|size_t|[iu]size)(?=\\s*\\()",
    ...
    "function-decl": {
      ...
      "match": "^(\\bpub\\b\\s+)?\\b(fn)\\b\\s+(?:\\([^)]+\\)\\s+)?(?:C\\.)?(\\w+)\\s*((?<=[+\\w\\s])(<)(\\w+)(>))?",
    ...
    "string-placeholder": {
      "match": "%(\\[\\d+])?([- #+0]{0,2}((\\d+|\\*)?(\\.?(\\d+|\\*|(\\[\\d+])\\*?)?(\\[\\d+])?)?))?[%EFGTUXb-gopqstvx]",

...
let json =
  ...
  "repository": {
    ...
    "event": {
      "begin": "\\b(event)\\b\\s+([_\\p{L}][_\\p{L}\\p{N}\\p{M}]*)\\s*",
      "beginCaptures": {
        "1": {
    ...
    "function": {
      "begin": "\\b(fun)\\b\\s+([_\\p{L}][_\\p{L}\\p{N}\\p{M}]*)\\s*",
      "beginCaptures": {
        "1": {
    ...
    "var-let-declaration": {
      "begin": "\\b(var|let)\\b\\s+([_\\p{L}][_\\p{L}\\p{N}\\p{M}]*)",
      "beginCaptures": {
        "1": {
          "name": "storage.type.$1.cadence"
        },

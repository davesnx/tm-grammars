  ...
  "repository": {
    "core_syntax": {
      "patterns": [
        ...
        {
          "begin": "^\\s*(def(?:|macro|delegate|guard))\\s+((?>[A-Z_a-z]\\w*(?>\\.|::))?(?>[A-Z_a-z]\\w*(?>[!?]|=(?!>))?|===?|>[=>]?|<=>|<[<=]?|[%\\&/`|]|\\*\\*?|=?~|[-+]@?|\\[]=?))((\\()|\\s*)",
          "beginCaptures": {
            "1": {
              "name": "keyword.control.module.elixir"
            },
          ...
          "end": "\\b(do:)|\\b(do)\\b|(?=\\s+(def(?:|n|macro|delegate|guard))\\b)",
          "endCaptures": {
            "1": {
        ...
        {
          "begin": "^\\s*(def(?:|n|macro|guard)p)\\s+((?>[A-Z_a-z]\\w*(?>\\.|::))?(?>[A-Z_a-z]\\w*(?>[!?]|=(?!>))?|===?|>[=>]?|<=>|<[<=]?|[%\\&/`|]|\\*\\*?|=?~|[-+]@?|\\[]=?))((\\()|\\s*)",
          "beginCaptures": {
            "1": {
              "name": "keyword.control.module.elixir"
            },
          ...
          "end": "\\b(do:)|\\b(do)\\b|(?=\\s+(def(?:p|macrop|guardp))\\b)",
          "endCaptures": {
            "1": {
              "name": "constant.other.keywords.elixir"
            },

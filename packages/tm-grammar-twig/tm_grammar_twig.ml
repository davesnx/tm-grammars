...
let json =
  ...
  "repository": {
    ...
    "twig-filters-warg": {
      "begin": "(?<=[]\"')0-9A-Z_a-z\\x7F-ÿ]\\||\\{%\\sfilter\\s)(batch|convert_encoding|date|date_modify|default|e(?:scape)?|format|join|merge|number_format|replace|round|slice|split|trim)(\\()",
      "beginCaptures": {
        "1": {
          "name": "support.function.twig"
        },

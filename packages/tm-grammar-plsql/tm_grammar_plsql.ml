...
let json = {json|{
  ...
  "patterns": [
    ...
    },
    {
      ...
      "match": "(?i)^\\s*(function|procedure)\\s+\"?([-0-9_a-z]+)\"?",
    ...
    {
      "match": "(?i)\\b(sql(?:|code))\\b",
      "name": "variable.language.oracle"
    },
    {
      "match": "(?i)\\b(ascii|asciistr|chr|compose|concat|convert|decompose|dump|initcap|instrb??|instrc|instr2|instr4|unistr|lengthb??|lengthc|length2|length4|lower|lpad|ltrim|nchr|replace|rpad|rtrim|soundex|substr|translate|trim|upper|vsize)\\b",
    ...
    {
      "match": "(?i)\\b(bfilename|cardinality|coalesce|decode|empty_([bc]lob)|lag|lead|listagg|lnnvl|nanvl|nullif|nvl2??|sys_(context|guid|typeid|connect_by_path|extract_utc)|uid|(current\\s+)?user|userenv|cardinality|(bulk\\s+)?collect|powermultiset(_by_cardinality)?|ora_hash|standard_hash|execute\\s+immediate|alter\\s+session)\\b",
    ...
    {
      "match": "(?i)\\b(abs|acos|asin|atan2??|bit_(and|or|xor)|ceil|cosh??|exp|extract|floor|greatest|least|ln|log|mod|power|remainder|round|sign|sinh??|sqrt|tanh??|trunc)\\b",
    ...
    {
      "match": "(?i)\\b(p([io]|io)_[-0-9_a-z]+)\\b",
      "name": "variable.parameter.oracle"
    },
    {
      "match": "(?i)\\b(l_[-0-9_a-z]+)\\b",
    ...
    {
      "match": "(?i)\\b(end|then|deterministic|exception|when|declare|begin|in|out|nocopy|is|as|exit|open|fetch|into|close|subtype|type|rowtype|default|exclusive|mode|lock|record|index\\s+by|result_cache|constant|comment|\\.((?:next|curr)val))\\b",
      "name": "keyword.other.oracle"
    },
    {
    ...
    {
      "match": "(?i)\\b(raise(?:|_application_error))\\b",
      "name": "support.function.oracle"
    },
    {
      "begin": "'",
    ...
    {
      "match": "(?i)\\b(char|varchar2??|nchar|nvarchar2|boolean|date|timestamp(\\s+with(\\s+local)?\\s+time\\s+zone)?|interval\\s*day(\\(\\d*\\))?\\s*to\\s*month|interval\\s*year(\\(\\d*\\))?\\s*to\\s*second(\\(\\d*\\))?|xmltype|blob|clob|nclob|bfile|long|long\\s+raw|raw|number|integer|decimal|smallint|float|binary_(float|double|integer)|pls_(float|double|integer)|rowid|urowid|vararray|naturaln??|positiven??|signtype|simple_(float|double|integer))\\b",

...
let json =
  ...
  "repository": {
    ...
    "keyword": {
      "patterns": [
        {
          "match": "\\b(if|while|for|return|au(g(?:|roup))|else(if|)?|do|in)\\b",
          "name": "keyword.control.viml"
        },
        {
          "match": "\\b(end(?:|if|for|while))\\s|$",
          "name": "keyword.control.viml"
        },
        {
        ...
        {
          "match": "\\b(func??|function|endfunction|endfunc)\\b",
    ...
    "storage": {
      "patterns": [
        ...
        {
          "match": "\\b(a(?:bort|utocmd))\\b",
          "name": "storage.viml"
        },
        {
          "match": "\\b(set(l(?:|ocal))?)\\b",
          "name": "storage.viml"
        },
        {
          "match": "\\b(com(mand)?)\\b",
    ...
    "support": {
      "patterns": [
        ...
        {
          "match": "(v:(beval_col|beval_bufnr|beval_lnum|beval_text|beval_winnr|char|charconvert_from|charconvert_to|cmdarg|cmdbang|count1??|ctype|dying|errmsg|exception|fcs_reason|fcs_choice|fname_in|fname_out|fname_new|fname_diff|folddashes|foldlevel|foldend|foldstart|insertmode|key|lang|lc_time|lnum|mouse_win|mouse_lnum|mouse_col|oldfiles|operator|prevcount|profiling|progname|register|scrollstart|servername|searchforward|shell_error|statusmsg|swapname|swapchoice|swapcommand|termresponse|this_session|throwpoint|val|version|warningmsg|windowid))",
    ...
    "syntax": {
      "patterns": [
        ...
        },
        {
          ...
          "match": "(syn(?:|tax))\\s+(cluster|keyword|match|region)(\\s+\\w+\\s+)(contained)?",
          "patterns": [
          ]
        },
        {
          ...
          "match": "(hi(?:|ghlight))\\s+(def(?:|ault))\\s+(link)\\s+(\\w+)\\s+(\\w+)",
          "patterns": [
          ]
        }
      ]

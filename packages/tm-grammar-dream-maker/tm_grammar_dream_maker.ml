...
let json = {json|{
  ...
  "patterns": [
    ...
    },
    {
      ...
      "match": "(var)[ /](?:(static|global|tmp|const)/)?(?:(datum|atom(?:/movable)?|obj|mob|turf|area|savefile|list|client|sound|image|database|matrix|regex|exception)/)?(?:([-$0-9A-Z_a-z]*)/)*([$0-9A-Z_a-z]*)\\b",
    ...
    {
      "begin": "^\\s*((#)\\s*define)\\s+((?<id>[A-Z_a-z][0-9A-Z_a-z]*))",
      "beginCaptures": {
        "1": {
          "name": "keyword.control.directive.define.dm"
        },
    ...
    {
      "begin": "(?:^|(?:(?=\\s)(?<!else|new|return)(?<=\\w)|(?=\\s*[A-Z_a-z])(?<!&&)(?<=[\\&*>])))(\\s*)(?!(while|for|do|if|else|switch|catch|enumerate|return|r?iterate)\\s*\\()((?:[A-Z_a-z][0-9A-Z_a-z]*+|::)++|(?<=operator)(?:[-!\\&*+<=>]+|\\(\\)|\\[]))\\s*(?=\\()",
      "beginCaptures": {
        "1": {
  ...
  "repository": {
    ...
    "string_escaped_char": {
      "patterns": [
        {
          "match": "\\\\(h(?:(?:er|im)self|ers|im)|([STst]?he)|He|[Hh]is|[Aa]n?|(?:im)?proper|\\.\\.\\.|(?:icon|ref|[Rr]oman)(?=\\[)|[\\n \"<>\\[ns])",

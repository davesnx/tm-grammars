...
let json = {json|{
  ...
  "patterns": [
    ...
    {
      "match": "(?<![.\\w\\d])0[Xx]\\.\\h+([Ee]-?\\d*)?([Pp][-+]\\d+)?",
      "name": "constant.numeric.float.hexadecimal.lua"
    },
    {
      "match": "(?<![.\\w\\d])0[Xx]\\h+(?![.0-9EPep])",
    ...
    },
    {
      ...
      "match": "\\b(goto)\\s+([A-Z_a-z][0-9A-Z_a-z]*)"
    },
    {
      "captures": {
        "1": {
    ...
    {
      "match": "(?<![^.]\\.|:)\\b(false|nil(?!:)|true|_ENV|_G|_VERSION|math\\.(pi|huge|maxinteger|mininteger)|utf8\\.charpattern|io\\.(std(?:in|out|err))|package\\.(config|cpath|loaded|loaders|path|preload|searchers))\\b|(?<!\\.)\\.{3}(?!\\.)",
      "name": "constant.language.lua"
    },
    {
      "match": "(?<![^.]\\.|:)\\b(self)\\b",
    ...
    {
      "match": "(?<=[^.]\\.|:)\\b([A-Z_a-z][0-9A-Z_a-z]*)\\b(?!\\s*=\\s*\\b(function)\\b)",

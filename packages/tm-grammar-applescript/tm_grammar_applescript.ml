  ...
  "repository": {
    ...
    "built-in.constant": {
      "patterns": [
        ...
        {
          "match": "\\b(?i:all\\s+(caps|lowercase)|bold|condensed|expanded|hidden|italic|outline|plain|shadow|small\\s+caps|strikethrough|(su(?:b|per))script|underline)\\b",
    ...
    "built-in.keyword": {
      "patterns": [
        ...
        {
          "match": "(?i)\\b(and|or|div|mod|as|not|(a\\s+)?(ref(?:(\\s+to)?|erence\\s+to))|equal(s|\\s+to)|contains?|comes\\s+(after|before)|(start|begin|end)s?\\s+with)\\b",
    ...
    "data-structures": {
      "patterns": [
        ...
        },
        {
          ...
          "match": "(«)(data) (ut(?:xt|f8))(\\h*)(»)(?:\\s+(as)\\s+(?i:Unicode\\s+text))?",
    ...
    "standardadditions": {
      "patterns": [
        ...
        {
          "match": "\\b(opening folder|((?:clos|mov)ing) folder window for|adding folder items to|removing folder items from)\\b",
    ...
    "system-events": {
      "patterns": [
        ...
        {
          "match": "\\b(((application |desk accessory )?process|(c(?:heck|ombo ))?box)(es)?|(action|attribute|browser|(busy|progress|relevance) indicator|color well|column|drawer|group|grow area|image|incrementor|list|menu( bar)?( item)?|(menu |pop up |radio )?button|outline|(radio|tab|splitter) group|row|scroll (area|bar)|sheet|slider|splitter|static text|table|text (area|field)|tool bar|UI element|window)s?)\\b",

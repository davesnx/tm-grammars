...
let json =
  ...
  "patterns": [
    ...
    {
      "match": "(?<=:\\s?)(?i:BINORMAL[0-9]*|BLENDINDICES[0-9]*|BLENDWEIGHT[0-9]*|COLOR[0-9]*|NORMAL[0-9]*|POSITIONT?|PSIZE[0-9]*|TANGENT[0-9]*|TEXCOORD[0-9]*|FOG|TESSFACTOR[0-9]*|VFACE|VPOS|DEPTH[0-9]*)\\b",
      "name": "support.variable.semantic.hlsl"
    },
    {
      "match": "(?<=:\\s?)(?i:SV_(?:ClipDistance[0-9]*|CullDistance[0-9]*|Coverage|Depth|DepthGreaterEqual[0-9]*|DepthLessEqual[0-9]*|InstanceID|IsFrontFace|Position|RenderTargetArrayIndex|SampleIndex|StencilRef|Target[0-7]?|VertexID|ViewportArrayIndex))\\b",
      "name": "support.variable.semantic.sm4.hlsl"
    },
    {
      "match": "(?<=:\\s?)(?i:SV_(?:DispatchThreadID|DomainLocation|GroupID|GroupIndex|GroupThreadID|GSInstanceID|InsideTessFactor|OutputControlPointID|TessFactor))\\b",
      "name": "support.variable.semantic.sm5.hlsl"
    },
    {
      "match": "(?<=:\\s?)(?i:SV_(?:InnerCoverage|StencilRef))\\b",
    ...
    {
      "match": "\\b(RasterizerOrdered(?:Buffer|ByteAddressBuffer|StructuredBuffer|Texture1D|Texture1DArray|Texture2D|Texture2DArray|Texture3D))\\b",
      "name": "support.type.object.rasterizerordered.hlsl"
    },
    {
      "match": "\\b(RW(?:Buffer|ByteAddressBuffer|StructuredBuffer|Texture1D|Texture1DArray|Texture2D|Texture2DArray|Texture3D))\\b",
      "name": "support.type.object.rw.hlsl"
    },
    {
      "match": "\\b((?:Line|Point|Triangle)Stream)\\b",
      "name": "support.type.object.geometryshader.hlsl"
    },
    {
      "match": "\\b(sampler(?:|1D|2D|3D|CUBE|_state))\\b",
      "name": "support.type.sampler.legacy.hlsl"
    },
    {
      "match": "\\b(Sampler(?:|Comparison)State)\\b",
      "name": "support.type.sampler.hlsl"
    },
    {
      "match": "\\b(texture(?:2D|CUBE))\\b",
      "name": "support.type.texture.legacy.hlsl"
    },
    {
      "match": "\\b(Texture(?:1D|1DArray|2D|2DArray|2DMS|2DMSArray|3D|Cube|CubeArray))\\b",
    ...
    {
      "match": "\\b((?:Blend|DepthStencil|Rasterizer)State)\\b",

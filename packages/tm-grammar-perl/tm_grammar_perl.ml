...
let json =
  ...
  "patterns": [
    ...
    {
      "begin": "\\b(?=s([^(0-9<A-\\[a-{\\s]).*\\1([acdegil-prsux]*)([),;}]|\\s+))",
      "end": "((([acdegil-prsux]*)))(?=([),;}]|\\s+|\\s*$))",
      "endCaptures": {
        "1": {
          "name": "string.regexp.replace.perl"
        },
    ...
    {
      "begin": "\\b(?=(?<!\\\\)s\\s*([^(<>\\[{\\s\\w]))",
      "end": "((([acdegilmoprsu]*x[acdegilmoprsu]*)))\\b",
      "endCaptures": {
        "1": {
          "name": "string.regexp.replace.perl"
        },
      ...
      "patterns": [
        ...
        },
        {
          ...
          "end": "\\1(?=[acdegilmoprsu]*x[acdegilmoprsu]*)\\b",
          "name": "string.regexp.replace.extended.simple_delimiter.perl",
          "patterns": [
            {
              "include": "#escaped_char"
    ...
    },
    {
      ...
      "match": "\\b(tr|y)\\s*([^0-9A-Za-z\\s])(.*?)(?<!\\\\)(\\\\{2})*(\\2)(.*?)(?<!\\\\)(\\\\{2})*(\\2)",
      "name": "string.regexp.replace.perl"
    },
    {
      "match": "\\b(__(?:FILE|LINE|PACKAGE|SUB)__)\\b",
    ...
    {
      "match": "(?<!\\w)-[ABCMORSTWXb-gklopr-uwxz]\\b",

...
let json = {json|{
  ...
  "patterns": [
    ...
    },
    {
      ...
      "match": "\\b(binary_(?:and|or|shift_left|shift_right|xor))(?:\\s*\\(\\s*(\\w+)\\s*,\\s*(\\w+)\\s*\\))?(?!\\w)",
    ...
    },
    {
      ...
      "match": "\\b(bi(?:nary_not|tset_count_ones))(?:\\s*\\(\\s*(\\w+)\\s*\\))?(?!\\w)",
      "name": "meta.scalar.bitwise.kusto"
    },
    {
      "match": "(?<!\\w)(!?in~?)(?!\\w)",
    ...
    },
    {
      ...
      "match": "\\b(range)\\s*\\((?:\\s*(\\w+(?:\\(.*?\\))?)\\s*,\\s*(\\w+(?:\\(.*?\\))?)\\s*,?\\s*{0,1}(\\w+(?:\\(.*?\\))?)?\\s*\\))?(?!\\w)",
    ...
    {
      "match": "\\b(series_(?:abs|acos|add|asin|atan|cos|decompose|decompose_anomalies|decompose_forecast|divide|equals|exp|fft|fill_backward|fill_const|fill_forward|fill_linear|fir|fit_2lines_dynamic|fit_2lines|fit_line_dynamic|fit_line|fit_poly|greater_equals|greater|ifft|iir|less_equals|less|multiply|not_equals|outliers|pearson_correlation|periods_detect|periods_validate|pow|seasonal|sign|sin|stats|stats_dynamic|subtract|tan))\\b",
      "name": "support.function.kusto"
    },
    {
      "match": "\\b(bag|array)\\b",
  ...
  "repository": {
    ...
    "Numeric": {
      "patterns": [
        {
          "match": "\\b((0([Xx])\\h*)|(([0-9]+\\.?[0-9]*+)|(\\.[0-9]+))(([Ee])([-+])?[0-9]+)?)([Ll]|UL|ul|[FUfu]|ll|LL|ull|ULL)?(?=\\b|\\w)",
    ...
    "TimeSpanLiterals": {
      "patterns": [
        {
          "match": "[-+]?(?:\\d*\\.)?\\d+(?:microseconds?|ticks?|seconds?|ms|[dhms])\\b",
          "name": "constant.numeric.kusto"
        }
      ]
    }

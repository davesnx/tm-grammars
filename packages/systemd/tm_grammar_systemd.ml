...
let json =
  ...
  "patterns": [
    ...
    {
      "begin": "^\\s*(Exec(?:Start(?:P(?:re|ost))?|Reload|Stop(?:Post)?))\\s*(=)[\\t ]*",
      "beginCaptures": {
        "1": {
          "name": "entity.name.tag"
        },
  ...
  "repository": {
      ...
      "patterns": [
        {
          ...
          "patterns": [
            {
              "match": "\\\\(?:[\\n\"'\\\\abfnrstv]|x\\h{2}|[0-8]{3}|u\\h{4}|U\\h{8})",
    ...
    "timeSpans": {
      "patterns": [
        {
          "match": "\\b(?:\\d+(?:[uμ]s(?:ec)?|ms(?:ec)?|s(?:ec(?:|onds?))?|m(?:in(?:|utes?))?|h(?:r|ours?)?|d(?:ays?)?|w(?:eeks)?|M|months?|y(?:ears?)?))+\\b",
          "name": "constant.numeric"
        }
      ]
    },
    ...
    "variables": {
      "patterns": [
        ...
        {
          "match": "%[ABCEG-JLMNPS-Wabf-jl-ps-w]\\b",
          "name": "constant.other.placeholder"
        }
      ]
    }

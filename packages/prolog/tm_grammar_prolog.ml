...
let json =
  ...
  "repository": {
    "atom": {
      "patterns": [
        {
          "match": "(?<![0-9A-Z_a-z])[a-z][0-9A-Z_a-z]*(?!\\s*\\(|[0-9A-Z_a-z])",
    ...
    "variable": {
      "patterns": [
        {
          "match": "(?<![0-9A-Z_a-z])[A-Z][0-9A-Z_a-z]*",
